...
export default function InternalSettingsForm() {
      ...
      <div className="max-w-2xl">
        ...
        <form onSubmit={submit} className="mt-10 rounded-2xl border border-[#E4E4E4] bg-[#FAFAFA] p-6 md:p-8 space-y-5">
          ...
          <label className="block">
            ...
            <div className="relative">
              <input required minLength={12} maxLength={256} type={visible.new ? 'text' : 'password'} autoComplete="new-password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full rounded-xl border border-[#D5D5D5] bg-white px-4 py-3 pr-12 outline-none focus:border-[#111111]" />
            ...
            <span className="mt-2 block text-xs text-[#777777]">Ít nhất 12 ký tự, tối đa 256 ký tự.</span>
          </label>
          <label className="block">
            <span className="mb-2 block text-sm font-medium">Xác nhận mật khẩu mới</span>
            <div className="relative">
              <input required minLength={12} maxLength={256} type={visible.confirm ? 'text' : 'password'} autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full rounded-xl border border-[#D5D5D5] bg-white px-4 py-3 pr-12 outline-none focus:border-[#111111]" />
